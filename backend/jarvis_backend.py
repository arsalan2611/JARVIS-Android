...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path=="/health":
            return self.reply(200,json.dumps({"ok":True,"service":"jarvis-backend","openai_configured":bool(OPENAI_KEY)}))
        return self.reply(404,'{"ok":false}')
    def do_POST(self):
        if self.path!="/v1/responses": return self.reply(404,'{"ok":false}')
        if not CLIENT_TOKEN or self.headers.get("Authorization","")!="Bearer "+CLIENT_TOKEN:
            return self.reply(401,'{"error":"unauthorized"}')
        if not OPENAI_KEY:
            return self.reply(503,'{"error":"OPENAI_API_KEY not configured"}')
        try:
            n=int(self.headers.get("Content-Length","0"))
            if n<=0 or n>2_000_000: return self.reply(413,'{"error":"invalid_payload_size"}')
            ...
            self.reply(502,json.dumps({"error":type(e).__name__}))
