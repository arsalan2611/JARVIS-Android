...
public class JarvisSetupActivity extends Activity {
    ...
    private void build(){
        ...
        LinearLayout routines=card();routines.addView(label("ROUTINES"));TextView rn=tv("Morning Brief و یادآوری‌های دوره‌ای از مسیر Reminder Engine خود JARVIS مدیریت می‌شوند.",11);rn.setTextColor(muted);rn.setPadding(0,0,0,dp(8));routines.addView(rn);Button routineBtn=btn("مدیریت Routineها");routineBtn.setOnClickListener(v->startActivity(new Intent(this,RoutineCenterActivity.class)));routines.addView(routineBtn,new LinearLayout.LayoutParams(-1,dp(48)));root.addView(routines);
