...
public class JarvisSetupActivity extends Activity {
    ...
    private void build(){
        ...
        TextView note=tv("Secretها در Android Keystore نگهداری می‌شوند. برای استفاده روزمره، Production Mode با Backend امن توصیه می‌شود؛ API Key را داخل APK یا چت وارد نکن.",11);note.setTextColor(muted);note.setBackground(shape(surface,18));root.addView(note);
