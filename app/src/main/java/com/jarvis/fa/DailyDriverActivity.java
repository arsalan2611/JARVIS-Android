...
public class DailyDriverActivity extends Activity {
    ...
    private TextView score,status,checklist;
    ...
    private void build(){
        ScrollView sc=new ScrollView(this);LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(dp(18),dp(20),dp(18),dp(30));root.setBackgroundColor(bg);root.setLayoutDirection(View.LAYOUT_DIRECTION_RTL);
        TextView title=tv("DAILY DRIVER",26);title.setTextColor(accent);title.setLetterSpacing(.12f);root.addView(title);
        TextView sub=tv("آماده‌سازی نهایی JARVIS برای استفاده روزمره",12);sub.setTextColor(muted);root.addView(sub);
        LinearLayout hero=card();score=tv("",34);score.setGravity(Gravity.CENTER);score.setTextColor(accent);hero.addView(score);status=tv("",14);status.setGravity(Gravity.CENTER);hero.addView(status);root.addView(hero);
        ...
        TextView note=tv("هدف این صفحه این است که به‌جای حدس، دقیقاً نشان دهد برای Daily Driver شدن چه چیزی روی همین گوشی باقی مانده. اتصال‌های خارجی فقط وقتی واقعاً تنظیم شده باشند آماده محسوب می‌شوند.",11);note.setTextColor(muted);root.addView(note);sc.addView(root);setContentView(sc);
    }

    private void refresh(){
        report=new AssistantReadiness(this).inspect();score.setText(report.score+"%");score.setTextColor(report.score>=90?good:(report.score>=70?accent:warn));
        status.setText(report.score>=90?"JARVIS برای استفاده روزمره تقریباً آماده است":report.score>=70?"چند تنظیم نهایی باقی مانده":"هنوز چند مورد ضروری باید آماده شود");
        StringBuilder b=new StringBuilder();
        line(b,"دستیار اصلی Android",report.roleHeld&&report.activeVoiceService);
        line(b,"میکروفون",report.micGranted);line(b,"تشخیص گفتار",report.speechAvailable);line(b,"اعلان‌ها",report.notificationsGranted);line(b,"یادآوری دقیق",report.exactAlarmsReady);line(b,"Backend امن HTTPS",report.secureBackendReady);line(b,"بدون Crash بررسی‌نشده",report.noPendingCrash);
    ...
    private void fixNext(){
        ...
        if(!report.roleHeld){requestRole();return;}
        if(!report.activeVoiceService){requestRole();return;}
        ...
        Toast.makeText(this,"موارد اصلی Daily Driver آماده‌اند.",Toast.LENGTH_SHORT).show();
    }
    private void requestRole(){
        if(Build.VERSION.SDK_INT>=29){try{RoleManager rm=(RoleManager)getSystemService(ROLE_SERVICE);if(rm!=null&&rm.isRoleAvailable(RoleManager.ROLE_ASSISTANT)){startActivityForResult(rm.createRequestRoleIntent(RoleManager.ROLE_ASSISTANT),REQ_ROLE);return;}}catch(Exception ignored){}}
